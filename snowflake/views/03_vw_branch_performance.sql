 ...
 * - SEL → SELECT
 * - FORMAT in expressions → removed (display formatting at client level)
 * - TRIM(col (FORMAT '9999')) → CAST(col AS VARCHAR)
 * - CSUM(expr, order_col) → SUM(expr) OVER (PARTITION BY ... ORDER BY ... ROWS UNBOUNDED PRECEDING)
 * - MAVG(expr, n, order_col) → AVG(expr) OVER (PARTITION BY ... ORDER BY ... ROWS BETWEEN n-1 PRECEDING AND CURRENT ROW)
 * - NULLIFZERO → NULLIFZERO (natively supported in Snowflake)
