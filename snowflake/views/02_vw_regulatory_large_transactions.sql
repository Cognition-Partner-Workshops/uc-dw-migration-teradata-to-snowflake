 ...
 *         SEL, QUALIFY, HASHROW, inline (NOT CASESPECIFIC).
 *
 * Conversion notes:
 *   - REPLACE VIEW converted to CREATE OR REPLACE VIEW.
 *   - LOCKING ROW FOR ACCESS removed (Snowflake MVCC).
 *   - SEL converted to SELECT.
 *   - Inline column attribute (NOT CASESPECIFIC) removed from SELECT;
 *     Snowflake is case-insensitive by default.
 *   - HASHROW(col1, col2) converted to HASH(col1, col2).
 *   - QUALIFY retained (natively supported in Snowflake).
 *   - COMMENT ON syntax retained.
 **********************************************************************/

CREATE OR REPLACE VIEW BANKING_DW.VW_REGULATORY_LARGE_TRANSACTIONS AS
SELECT
    ft.TRANSACTION_ID,
    ft.TRANSACTION_DATE,
    ft.TRANSACTION_TS,
