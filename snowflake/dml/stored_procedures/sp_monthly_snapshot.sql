 ...
 *   - Replaced VOLATILE TABLE with CREATE TEMPORARY TABLE
 *   - Replaced SEL with SELECT
 *   - Replaced FORMAT casts with TO_CHAR / TO_DATE
 *   - Replaced ADD_MONTHS with DATEADD
 *   - Replaced ACTIVITY_COUNT with SQLROWCOUNT
 *   - Removed PRIMARY INDEX on temp table (not needed in Snowflake)
 *   - Removed ON COMMIT PRESERVE ROWS (Snowflake temp tables persist within session)
 *   - Removed COLLECT STATISTICS
 *   - ZEROIFNULL kept (natively supported)
 *   - MERGE syntax kept (natively supported in Snowflake)
 *   - Adjusted type casts to Snowflake syntax
 **********************************************************************/
...
BEGIN
    ...
    v_period_start := TO_DATE(
        :P_SNAPSHOT_YEAR::VARCHAR || '-' ||
        LPAD(:P_SNAPSHOT_MONTH::VARCHAR, 2, '0') || '-01',
        'YYYY-MM-DD');
    ...
    CREATE OR REPLACE TEMPORARY TABLE BANKING_DW.TMP_TXN_AGGREGATES AS
    SELECT
        ft.ACCOUNT_KEY,
    ...
    USING (
        ...
        LEFT JOIN BANKING_DW.TMP_TXN_AGGREGATES agg
    ...
    DROP TABLE IF EXISTS BANKING_DW.TMP_TXN_AGGREGATES;
