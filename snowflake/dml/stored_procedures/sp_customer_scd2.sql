 ...
 *   - Converted to Snowflake SQL scripting (EXECUTE IMMEDIATE / Snowflake Scripting)
 *   - Replaced ACTIVITY_COUNT with SQLROWCOUNT
 *   - Replaced SEL with SELECT
 *   - Removed COLLECT STATISTICS
 *   - Replaced Teradata UPDATE...FROM syntax with standard UPDATE...SET...WHERE EXISTS
 *   - Adjusted TIMESTAMP syntax
    ...
    LEFT JOIN (
        SELECT CUSTOMER_ID, ONBOARDING_DATE,
               ROW_NUMBER() OVER (PARTITION BY CUSTOMER_ID
                                   ORDER BY EFFECTIVE_TO DESC) AS rn
