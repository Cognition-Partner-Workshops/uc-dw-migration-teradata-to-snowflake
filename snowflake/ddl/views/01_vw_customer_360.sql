 ...
 *   - LOCKING ROW FOR ACCESS -> removed (no equivalent needed)
 *   - SEL -> SELECT
 *   - ZEROIFNULL() -> ZEROIFNULL() (natively supported in Snowflake)
 *   - (FORMAT '...') inline formatting -> TO_CHAR() for display
 *   - Scalar subquery in CASE inside aggregate rewritten as lateral join
 *   - QUALIFY ROW_NUMBER() -> QUALIFY (natively supported in Snowflake)
 *   - Date arithmetic: (CURRENT_DATE - date) / 365.25 -> DATEDIFF
 **********************************************************************/
