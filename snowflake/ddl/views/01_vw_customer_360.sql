 ...
 * - Replaced: SEL -> SELECT
 * - Removed: (FORMAT '...') inline formatting
 * - Rewrote: Correlated scalar subquery with QUALIFY -> CTE with ROW_NUMBER
...
WITH latest_snapshot AS (
    SELECT
        ...
        ROW_NUMBER() OVER (PARTITION BY snap.ACCOUNT_KEY
                           ORDER BY snap.SNAPSHOT_DATE DESC) AS rn
