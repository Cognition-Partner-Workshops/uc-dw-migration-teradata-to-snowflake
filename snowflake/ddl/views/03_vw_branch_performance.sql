 ...
 *   - SEL -> SELECT
 *   - TRIM(col (FORMAT '9999')) -> TO_CHAR(col)
 *   - (FORMAT '...') inline -> removed (use TO_CHAR in reporting layer)
 *   - CSUM(expr, order_col) -> SUM(expr) OVER (PARTITION BY ... ORDER BY ...)
 *   - MAVG(expr, N, order_col) -> AVG(expr) OVER (... ROWS BETWEEN N-1 PRECEDING AND CURRENT ROW)
 *   - NULLIFZERO() -> NULLIFZERO() (natively supported)
 *   - ADD_MONTHS() -> DATEADD('month', -24, CURRENT_DATE)
 **********************************************************************/
...
SELECT
    ...
    /* Teradata CSUM -> Snowflake SUM() OVER window function */
    ...
    /* Teradata MAVG(expr, 3, order) -> Snowflake AVG() OVER with 3-row window */
