 ...
 *
 * Uses OLAP / window functions for cumulative sums and moving averages.
 * Converted from Teradata view.
 * - REPLACE VIEW  ->  CREATE OR REPLACE VIEW
 * - LOCKING ROW FOR ACCESS  ->  removed
 * - SEL  ->  SELECT
 * - Inline FORMAT clauses removed
 * - TRIM(col (FORMAT 'pattern'))  ->  TO_CHAR(col, 'pattern')
 * - CSUM(expr, order_col)  ->  SUM(expr) OVER (ORDER BY order_col ROWS UNBOUNDED PRECEDING)
 * - MAVG(expr, n, order_col)  ->  AVG(expr) OVER (ORDER BY order_col ROWS BETWEEN (n-1) PRECEDING AND CURRENT ROW)
 * - NULLIFZERO kept (Snowflake supports natively)
 * - ADD_MONTHS kept (Snowflake supports natively)
 **********************************************************************/
...
SELECT
    ...
    d.MONTH_NAME || ' ' || TO_CHAR(d.CALENDAR_YEAR, '9999') AS MONTH_LABEL,
    ...
    /* Snowflake replacement for Teradata CSUM -- cumulative sum */
    SUM(SUM(snap.FEES_CHARGED)) OVER (
        PARTITION BY b.BRANCH_ID
        ORDER BY snap.SNAPSHOT_MONTH_KEY
        ROWS UNBOUNDED PRECEDING
    ) AS CUMULATIVE_FEES_YTD,
    /* Snowflake replacement for Teradata MAVG -- 3-month moving average */
    ...
    SUM(snap.CLOSING_BALANCE) /
        NULLIFZERO(SUM(SUM(snap.CLOSING_BALANCE))
                   OVER (PARTITION BY b.REGION, snap.SNAPSHOT_MONTH_KEY))
